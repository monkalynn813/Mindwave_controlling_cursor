    ...
    def filter(self,sample):
        ...
        if len(self.raw_data)!= 0 and len(self.raw_data) %self.frame ==0:
            ...
            for k in range(self.channelnum):
                ...
                freq,y=self.fft(channel_filtered,self.fs)
                ##insert figure here?
                ##Reduce dimension of FFT data
                freq_ind=np.where((freq>=self.band[0])&(freq<=self.band[1]))[0] #take diresed frequency
                desired_freq=freq[freq_ind]
                amp_of_desired_freq=y[freq_ind]
                average_amp_desired=np.mean(amp_of_desired_freq)
